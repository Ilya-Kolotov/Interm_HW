...
while True:
    ...
    if command == "read":
        ...
        start_date = datetime.datetime.strptime(input("Введите начальную дату (в формате ГГГГ-ММ-ДД): "),
                                                "%Y-%m-%d").date()
        end_date = datetime.datetime.strptime(input("Введите конечную дату (в формате ГГГГ-ММ-ДД): "),
                                              "%Y-%m-%d").date()
